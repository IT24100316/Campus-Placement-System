...
class ApiService {
  Future<Map<String, dynamic>> registerStudent({
    ...
    if (campusId.bytes != null) {
      request.files.add(http.MultipartFile.fromBytes('campusIdPhoto', campusId.bytes!, filename: campusId.name));
    } else if (campusId.path != null) {
      request.files.add(await http.MultipartFile.fromPath('campusIdPhoto', campusId.path!, filename: campusId.name));
    } else {
